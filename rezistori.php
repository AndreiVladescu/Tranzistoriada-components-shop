        ...
        <div class="row">
                            ...
                            <tr>
                                <th>
                                    <div class="product-filter-item-container" style="overflow:auto; height:6cm">
                                        ...
                                        <?php
                                        $sql = "select distinct Producer from resistors";
                                ...
                                <th>
                                    <div class="product-filter-item-container" style="overflow:auto; height:6cm">
                                        ...
                                        <?php
                                        $sql = "select distinct Type from resistors";
                                        $result = mysqli_query($conn, $sql);
                                        $counter = 0;
                                        while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                                ...
                                <th>
                                    <div class="product-filter-item-container" style="overflow:auto; height:6cm">
                                        ...
                                        <?php
                                        $sql = "select distinct Resistance from resistors order by Resistance ";
                                        $result = mysqli_query($conn, $sql);
                                        $counter = 0;
                                        while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                                ...
                                <th>
                                    <div class="product-filter-item-container" style="overflow:auto; height:6cm">
                                        ...
                                        <?php
                                        $sql = "select distinct Power from resistors order by Power";
                                        $result = mysqli_query($conn, $sql);
                                        $counter = 0;
                                        while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                            ...
                            <tr>
                                <th>
                                    ...
                                    <div class="product-filter-item-container" style="overflow:auto; height:6cm">
                                        ...
                                        <?php
                                        $sql = "select distinct Case_dimensions from resistors";
                                        $result = mysqli_query($conn, $sql);
                                        $counter = 0;
                                        while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                                            echo "<div class=\"form-check\"><input class=\"form-check-input\" type='checkbox' name='case_dimensions[]' value='" . $row['Case_dimensions'] . "' id=\"case_dimensions_" . $counter . "\"/>"
                                                . "<label class=\"form-check-label\" for=\"case_dimensions_" . $counter . "\">" . $row['Case_dimensions'] . "</label></div>";
                                            $counter++;
                                        }
                                        ?>
                                    </div>
                                ...
                                <th>
                                    <div class="product-filter-item-container" style="overflow:auto; height:6cm">
                                        ...
                                        <?php
                                        $sql = "select distinct Terminal_dimensions from resistors";
                                        $result = mysqli_query($conn, $sql);
                                        $counter = 0;
                                        while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                                            echo "<div class=\"form-check\"><input class=\"form-check-input\" type='checkbox' name='terminal_dimensions[]' value='" . $row['Terminal_dimensions'] . "' id=\"terminal_dimensions" . $counter . "\"/>"
                                                . "<label class=\"form-check-label\" for=\"terminal_dimensions" . $counter . "\">" . $row['Terminal_dimensions'] . "</label></div>";
                                            $counter++;
                                        }
                                        ?>
                                    </div>
                    ...
                    if (isset($_POST['filter'])) {
                        ...
                        if (!empty($_POST['power'])) {
                            foreach ($_POST['power'] as $value) {
                                array_push($filtre[3], "Power = " . $value);
                            }
                        }
                        if (!empty($_POST['resistance'])) {
                            foreach ($_POST['resistance'] as $value) {
                                array_push($filtre[2], "Resistance = " . $value);
                            }
                        }
                        if (!empty($_POST['case_dimensions'])) {
                            foreach ($_POST['case_dimensions'] as $value) {
                                array_push($filtre[4], "Case_dimensions = '" . $value . "'");
                            }
                        }
                        if (!empty($_POST['terminal_dimensions'])) {
                            foreach ($_POST['terminal_dimensions'] as $value) {
                                array_push($filtre[5], "Terminal_dimensions = '" . $value . "'");
                            }
                        }
                    }
                    $prev_count = count($filtre[0]) + count($filtre[1]) + count($filtre[2]) + count($filtre[3]) + count($filtre[4]); // + count($filtre[5]);
            ...
            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    $tmp_image = "images/componente/rezistori/";
                    switch ($row["Type"]) {
                        case "Bobinat":
                            $tmp_image = $tmp_image . "bobinat.png";
                            break;
                        case "Carbon":
                            $tmp_image = $tmp_image . "carbon.png";
                            break;
                        case "Metal-Oxid":
                            $tmp_image = $tmp_image . "metal.png";
                            break;
                        case "Miez-sarma":
                            $tmp_image = $tmp_image . "miez.png";
                            break;
                        case "Nicrom":
                            $tmp_image = $tmp_image . "nicrom.png";
                            break;
                        default:
                            $tmp_image = $tmp_image . "nicrom.png";
                            break;
                    ...
                    echo "<tr><h5 align=\"left\">Rezistor: " . $row["Type"] . "; " . $row["Resistance"] . " Ω; " . $row["Power"] . " W; " . $row["Case_dimensions"] . "; " . $row["Terminal_dimensions"] . "; " . "</h5></tr>";
                    ...
                    echo "<th><h5>Multiplu: 1<br><br><form action=\"\" method=\"post\"><input type=\"text\" id=\"resistor_" . $row["ID"] .
            ...
            if (!empty($_SESSION["mail"])) {
                if (!empty($_POST["ID"]) && $_POST["cantitate"] > 0) {
                    $sql = "select * from resistors where id = " . $_POST["ID"];

                    $result = mysqli_query($conn, $sql);
                    $row = mysqli_fetch_assoc($result);

                    $sql = "select Quantity from cart_item where ID_resistor = " . $row["ID"] . " and ID_client = " . $_SESSION["id"];
                    $resistor_id = $row["ID"];
                    $result = mysqli_query($conn, $sql);
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        ...
                        $sql = "delete from cart_item where ID_resistor = " . $resistor_id . " and ID_client = " . $_SESSION["id"];
                        //echo $sql;
                        ...
                        $row = mysqli_fetch_assoc($result);
                        $sql = "insert into cart_item (ID_client, ID_resistor, Product_name, Price, Quantity) values ("
                            . $_SESSION["id"] . ', '
                            . $row["ID"] . ', \''
                            . $row["Name"] . '\', '
                            . $row["Price"] . ', '
