        ...
        <div class="row">
                        ...
                        <table class="table">
                            <tr>
                                <th>
                                    <div class="product-filter-item-container" style="overflow:auto; height:6cm">
                                        ...
                                        <?php
                                        $sql = "select distinct Producer from capacitors";
                                ...
                                <th>
                                    <div class="product-filter-item-container" style="overflow:auto; height:6cm">
                                        ...
                                        <?php
                                        $sql = "select distinct Capacitor_type from capacitors";
                                        $result = mysqli_query($conn, $sql);
                                        $counter = 0;
                                        while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                                            echo "<div class=\"form-check\"><input class=\"form-check-input\" type='checkbox' name='type[]' value='" . $row['Capacitor_type'] . "' id=\"type_" . $counter . "\"/>"
                                                . "<label class=\"form-check-label\" for=\"type_" . $counter . "\">" . $row['Capacitor_type'] . "</label></div>";
                                            $counter++;
                                        }
                                        ?>
                                    </div>
                                ...
                                <th>
                                    <div class="product-filter-item-container" style="overflow:auto; height:6cm">
                                        ...
                                        <?php
                                        $sql = "select distinct Capacitance from capacitors order by Capacitance";
                                        $result = mysqli_query($conn, $sql);
                                        $counter = 0;
                                        while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                                            echo "<div class=\"form-check\"><input class=\"form-check-input\" type='checkbox' name='capacitance[]' value='" . $row['Capacitance'] . "' id=\"capacitance_" . $counter . "\"/>"
                                                . "<label class=\"form-check-label\" for=\"capacitance_" . $counter . "\">" . $row['Capacitance'] . " uF</label></div>";
                                            $counter++;
                                        }
                                        ?>
                                    </div>
                                ...
                                <th>
                                    ...
                                    <div class="product-filter-item-container" style="overflow:auto; height:6cm">
                                        ...
                                        <?php
                                        $sql = "select distinct Case_dimensions from capacitors";
                                        $result = mysqli_query($conn, $sql);
                                        $counter = 0;
                                        while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                                            echo "<div class=\"form-check\"><input class=\"form-check-input\" type='checkbox' name='case_dim[]' value='" . $row['Case_dimensions'] . "' id=\"case_dim_" . $counter . "\"/>"
                                                . "<label class=\"form-check-label\" for=\"case_dim_" . $counter . "\">" . $row['Case_dimensions'] . "</label></div>";
                                            $counter++;
                                        }
                                        ?>
                                    </div>
                            ...
                            <tr>
                                <th>
                                    <div class="product-filter-item-container" style="overflow:auto; height:6cm">
                                        <h5>
                                            Voltaj de Funcționare
                                        </h5>
                                        <?php
                                        $sql = "select distinct Working_voltage from capacitors order by Working_voltage";
                                        $result = mysqli_query($conn, $sql);
                                        $counter = 0;
                                        while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                                            echo "<div class=\"form-check\"><input class=\"form-check-input\" type='checkbox' name='voltage[]' value='" . $row['Working_voltage'] . "' id=\"voltage" . $counter . "\"/>"
                                                . "<label class=\"form-check-label\" for=\"voltage" . $counter . "\">" . $row['Working_voltage'] . " V</label></div>";
                                            $counter++;
                                        }
                                        ?>
                                    </div>
                    ...
                    if (isset($_POST['filter'])) {
                        ...
                        if (!empty($_POST['type'])) {
                            foreach ($_POST['type'] as $value) {
                                array_push($filtre[1], "Capacitor_type = '" . $value . "'");
                            }
                        }
                        if (!empty($_POST['capacitance'])) {
                            foreach ($_POST['capacitance'] as $value) {
                                array_push($filtre[2], "Capacitance = " . $value);
                            }
                        }
                        if (!empty($_POST['case_dim'])) {
                            foreach ($_POST['case_dim'] as $value) {
                                array_push($filtre[3], "Case_dimensions = '" . $value . "'");
                            }
                        }
                        if (!empty($_POST['voltage'])) {
                            foreach ($_POST['voltage'] as $value) {
                                array_push($filtre[4], "Working_voltage = " . $value);
                            }
                        }
                    }
            ...
            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    $tmp_image = "images/componente/condensatori/";
                    switch ($row["Capacitor_type"]) {
                        case "ceramic":
                            $tmp_image = $tmp_image . "ceramic.png";
                            break;
                        case "poliester":
                            $tmp_image = $tmp_image . "poliester.png";
                            break;
                        case "electrolitic":
                            $tmp_image = $tmp_image . "electrolitic.png";
                            break;
                        case "polipropilena":
                            $tmp_image = $tmp_image . "polipropilena.png";
                            break;
                        case "tantal":
                            $tmp_image = $tmp_image . "tantal.png";
                            break;
                        default:
                    ...
                    echo "<tr><h5 align=\"left\">Condensator: " . $row["Capacitor_type"] . "; " . $row["Capacitance"] . " uF; " . $row["Case_dimensions"] . "; " . $row["Working_voltage"] . " V</h5></tr>";
                    ...
                    echo "<th><h5>Multiplu: 1<br><br><form action=\"\" method=\"post\"><input type=\"text\" id=\"capacitor_" . $row["ID"] .
            ...
            if (!empty($_SESSION["mail"])) {
                if (!empty($_POST["ID"]) && $_POST["cantitate"] > 0) {
                    $sql = "select * from capacitors where id = " . $_POST["ID"];

                    $result = mysqli_query($conn, $sql);
                    $row = mysqli_fetch_assoc($result);

                    $sql = "select Quantity from cart_item where ID_capacitor = " . $row["ID"] . " and ID_client = " . $_SESSION["id"];
                    $capacitor_id = $row["ID"];
                    $result = mysqli_query($conn, $sql);
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        ...
                        $sql = "delete from cart_item where ID_capacitor = " . $capacitor_id . " and ID_client = " . $_SESSION["id"];
                        echo $sql;
                        ...
                        $row = mysqli_fetch_assoc($result);
                        $sql = "insert into cart_item (ID_client, ID_capacitor, Product_name, Price, Quantity) values ("
                            . $_SESSION["id"] . ', '
                            . $row["ID"] . ', \''
                            . $row["Name"] . '\', '
                            . $row["Price"] . ', '
